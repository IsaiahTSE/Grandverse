		...
		if (isset($action)){
			if ($action == 'delete_user') {
				...
				if ($user_result->num_rows == 0){
					exit('{"success":0,"problem":"Opps! We cannot find the user. Are you spelling the User ID right?"}');
				} else {
					$user = $user_result->fetch_assoc();
					if ($user['user_level'] > 0) {
						exit('{"success":0,"problem":"Admins cannot be deleted"}');
					} else {
                  ...
                  <div class="card-body">
                    ...
                    <form id="delete_post" action="/admin_panel/delete_post">
                      ...
                      <select name="post_violation_type" class="form-control">
                        ...
                        <option value="3">Inappropiate Language</option>
                        <option value="4">Hate Speech</option>
